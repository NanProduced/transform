        ...
        <div className="flex-1">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-muted/50 border border-border rounded-lg text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/50"
            />
          </div>
        </div>
        <button
          onClick={onClose}
          className="lg:hidden flex items-center justify-center w-9 h-9 rounded-lg hover:bg-muted transition-colors text-muted-foreground"
        >
                ...
                .map((item: Route) => {
                  ...
                  return (
                    <Link key={item.label} href={item.path} prefetch={false}>
                      ...
                      >
                        <div
                          className={`pl-6 pr-4 py-2 text-sm transition-colors cursor-pointer border-l-2 ${
                            isActive
    ...
    <>
      ...
      {isOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <div
            ref={sidebarRef}
            className="absolute left-0 top-0 bottom-0 w-80 max-w-[85vw] bg-card border-r border-border shadow-2xl animate-in slide-in-right duration-200"
